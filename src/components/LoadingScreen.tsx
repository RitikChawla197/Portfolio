...
const words = ['Hello', 'Bonjour', 'Hola', 'Namaste', 'Ciao']
...
const WordPreloader = ({ onFinish }: { onFinish: () => void }) => {
  ...
  useEffect(() => {
    if (currentWordIndex < words.length - 1) {
      const timer = setTimeout(() => setCurrentWordIndex((prev) => prev + 1), 200)
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center transition-all duration-700 ease-out ${
        isExiting ? 'bg-black opacity-0 scale-105' : 'bg-black opacity-100 scale-100'
      }`}
    >
      <div className="relative h-40 flex items-center justify-center">
        {words.map((word, index) => (
          ...
          >
            <h1 className="text-white text-6xl sm:text-7xl font-extrabold tracking-tight">
              {word}
            </h1>
          </div>
        ))}
